<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Editar Orçamento') }} - {{ $quote->quote_number }}
        </h2>
    </x-slot>
    ...
    <div class="py-2">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-150 p-8">
                ...
                <form action="{{ route('quotes.update', $quote) }}" method="POST" x-data="quoteForm()">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                        ...
                        <div>
                            ...
                            <select id="status" name="status" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                <option value="draft" {{ old('status', $quote->status) == 'draft' ? 'selected' : '' }}>{{ __('Rascunho') }}</option>
                                <option value="sent" {{ old('status', $quote->status) == 'sent' ? 'selected' : '' }}>{{ __('Enviado') }}</option>
                                <option value="approved" {{ old('status', $quote->status) == 'approved' ? 'selected' : '' }}>{{ __('Aprovado') }}</option>
                                <option value="declined" {{ old('status', $quote->status) == 'declined' ? 'selected' : '' }}>{{ __('Recusado') }}</option>
                            </select>
                        </div>

                        <div>
                            <x-input-label for="issue_date" :value="__('Data de Emissão')" />
                            <x-text-input id="issue_date" class="block mt-1 w-full" type="date" name="issue_date" value="{{ old('issue_date', $quote->issue_date) }}" required />
                        </div>

                        <div>
                            <x-input-label for="due_date" :value="__('Data de Vencimento')" />
                            <x-text-input id="due_date" class="block mt-1 w-full" type="date" name="due_date" value="{{ old('due_date', $quote->due_date) }}" />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
                        ...
                        <div class="md:col-span-8">
                            ...
                            <textarea id="notes" name="notes" rows="4" placeholder="{{ __('Termos de garantia, forma de pagamento, detalhes de entrega...') }}" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm">{{ old('notes', $quote->notes) }}</textarea>
                        </div>
    ...
    <script>
        function quoteForm() {
            return {
                // Load existing items mapped and formatted in BRL from PHP
